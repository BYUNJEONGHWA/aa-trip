  ...
  return (
    <header className="h-14 bg-white/95 backdrop-blur-md border-b border-slate-200/80 px-3 sm:px-6 flex items-center justify-between gap-2 z-30 shrink-0 shadow-xs">
      ...
      <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
        ...
        {/* Group 2: DB & API 연동 설정 (Slate Box with Connection Status & Auto-Save Badge) */}
        <div className="flex items-center gap-1 bg-slate-100/80 p-1 rounded-xl border border-slate-200">
          <button
            onClick={() => setIsSupabaseModalOpen(true)}
            className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-lg bg-white hover:bg-slate-50 text-slate-800 text-xs font-black transition-all shadow-2xs border border-slate-200/70"
          ...
          >
            ...
            {/* Auto Save Real-time Feedback Badges */}
            {autoSaveStatus === 'saving' && (
              <span className="ml-1 text-[10px] text-emerald-700 font-extrabold flex items-center gap-1 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200 animate-pulse">
                <RefreshCw className="w-3 h-3 animate-spin text-emerald-600" />
                <span className="hidden md:inline">자동 저장 중...</span>
              </span>
            )}
            {autoSaveStatus === 'saved' && (
              <span className="ml-1 text-[10px] text-emerald-700 font-extrabold flex items-center gap-1 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200">
                <Check className="w-3 h-3 text-emerald-600" />
                <span className="hidden md:inline">DB 자동저장됨</span>
              </span>
            )}
            {autoSaveStatus === 'error' && (
              <span className="ml-1 text-[10px] text-amber-700 font-extrabold flex items-center gap-1 bg-amber-50 px-2 py-0.5 rounded-md border border-amber-200">
                <AlertTriangle className="w-3 h-3 text-amber-600" />
                <span className="hidden md:inline">자동저장 실패</span>
              </span>
            )}
          </button>
          <button
